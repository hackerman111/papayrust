...
pub fn render_add_paper_modal(app: &App, frame: &mut Frame) {
    ...
    // 4. Help text
    let help_text = " Enter: Import PDF or Folder | Esc: Cancel ";
    let help_p = Paragraph::new(help_text)
        .style(Style::default().fg(Color::Cyan))
        .alignment(ratatui::layout::Alignment::Center);
...
pub fn render_help_modal(_app: &App, frame: &mut Frame) {
    ...
    let help_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::raw("j/k move | a new collection | d delete collection"),
        ]),
        Line::from(vec![
            Span::styled(
                "Papers Panel: ",
        ...
        Line::from(vec![
            Span::raw("              "),
            Span::raw("a add PDF/folder | e edit metadata | d delete paper"),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled(
